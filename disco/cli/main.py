...
from disco.data.loaders import (
    load_adult,
    load_synthetic_tabular,
    load_california_housing,  # 新增
    stack_prewindow,
)
...
def train_devices_cmd(args):
    ...
    if args.dataset == "scenario":
        ...
        if not args.scenario:
            raise ValueError(
                "--scenario path must be provided when dataset is 'scenario'"
            )
        ...
        for i in range(args.N):
            ...
            template_model = scenario_model_templates[i % len(scenario_model_templates)]
            model_params = template_model.get_params()
            if "random_state" in model_params:
                model_params["random_state"] = args.seed + i
            model = template_model.__class__(**model_params)
        ...
        registry = {
            "devices": [d.id for d in trained_devices],
            "dataset": "scenario",
            "scenario": str(scenario_path),
            "samples": args.samples,
            "seed": args.seed,
            "models": device_registry_entries,
        }

        with open(Path(args.out) / "registry.json", "w") as f:
            json.dump(registry, f, indent=2)

        print(f"Saved {len(trained_devices)} devices to {args.out}")
        return
    ...
    elif args.dataset == "california":  # 新增
    ...
    if args.dataset == "california":
        ...
        # 按 splits 循环训练 N 个回归设备
        for i, ((X_i, y_i), template_model) in enumerate(
            zip(
                splits,
                regression_model_templates
                * (args.N // len(regression_model_templates) + 1),
            )
        ):
            print(f"Device {i+1}: Training {template_model.__class__.__name__}")
            ...
            if len(X_test) > 100:
                ...
                mse = float(np.mean((preds - y_test[:100]) ** 2))
    ...
    classification_model_templates = [
    ...
    for i, ((X_i, y_i), template_model) in enumerate(
        zip(
            splits,
            classification_model_templates
            * (args.N // len(classification_model_templates) + 1),
        )
    ):
        print(f"Device {i+1}: Training {template_model.__class__.__name__}")

        # Clone model with different random state
        model = template_model.__class__(
            **{**template_model.get_params(), "random_state": args.seed + i}
        )

        model.fit(X_i, y_i)

        device = SklearnDevice(
            id=f"device_{i+1}", model=model, task="classification", scalarization="prob"
        )
        devices.append(device)
...
def load_devices(devices_dir: str) -> List:
    ...
    device_files = sorted(
        [
            ...
            if f.startswith("device_") and f.endswith(".pkl")
        ]
    )
...
def probe_cmd(args):
    ...
    target_class = (
        1 if getattr(devices[0], "task", "classification") == "classification" else 0
    )
...
def explain_cmd(args):
    ...
    if args.interventions:
        if args.interventions.endswith(".yaml"):
            ...
            interventions = [FeatureAdd(feat_idx=0, delta_max=3.0, name="add_f0")]
    else:
        interventions = [FeatureAdd(feat_idx=0, delta_max=3.0, name="add_f0")]
    ...
    query_indices = rng.choice(
        len(X_test), min(args.queries, len(X_test)), replace=False
    )

    print(
        f"Running explanations for {len(query_indices)} queries on {len(target_devices)} targets"
    )
            ...
            for intervention in interventions:
                ...
                try:
                    ...
                    diagnostics = {
                        ...
                        "flip_theta": (
                            float(result["te_output"].flip_theta)
                    ...
                    print(
                        f"Error processing target {t_idx}, query {q_idx}, intervention {intervention.name}: {e}"
                    )
                    continue

            if q_idx % 10 == 0:
                print(
                    f"  Completed {q_idx+1}/{len(query_indices)} queries for target {t_idx}"
                )
...
def run_disco_pipeline(
    ...
    from disco.matching.metrics import (
        ...
        compute_effective_peers,
        compute_weight_entropy,
    )
    from disco.anchors.knn import compute_anchor_radius

    # Anchor selection
    use_response_space = anchor_space == "response" and grids.theta0.size > 0
    ...
    y_t, X_peers = stack_prewindow(
        trajectories, target_device, devices, anchor_sel, intervention, grids.theta0
    )
...
def synth_cmd(args):
    ...
    print(f"Synthetic scenario saved to {out_dir}")


def main():
    """Main CLI entry point."""
    parser = argparse.ArgumentParser(
        description="DISCO: Distributed Synthetic Control Explanations"
    )
    subparsers = parser.add_subparsers(dest="command", help="Available commands")

    # Train devices
    train_parser = subparsers.add_parser(
        "train-devices", help="Train heterogeneous devices"
    )
    train_parser.add_argument(
        "--dataset", choices=["adult", "synthetic", "scenario", "california"], default="adult"
    )
    train_parser.add_argument("--N", type=int, default=5, help="Number of devices")
    train_parser.add_argument(
        "--non-iid",
        choices=["iid", "label-skew", "covariate-shift", "feature-shift"],
        default="label-skew",
    )
    train_parser.add_argument(
        "--out", required=True, help="Output directory for devices"
    )
    train_parser.add_argument("--seed", type=int, default=42)
    train_parser.add_argument(
        "--scenario", help="Path to synthetic scenario for scenario dataset"
    )
    train_parser.add_argument(
        "--samples",
        type=int,
        default=2000,
        help="Samples per device for scenario dataset",
    )

    # Probe
    probe_parser = subparsers.add_parser("probe", help="Compute probe trajectories")
    probe_parser.add_argument(
        "--devices", required=True, help="Directory with trained devices"
    )
    probe_parser.add_argument(
        "--dataset", choices=["adult", "synthetic", "scenario", "california"], default="adult"
    )
    probe_parser.add_argument(
        "--probes", type=int, default=500, help="Number of probe samples"
    )
    probe_parser.add_argument(
        "--theta0", default="[0.0,0.5,1.0]", help="Pre-window intensities"
    )
    ...
    probe_parser.add_argument(
        "--dp-noise-std", type=float, default=0.0, help="DP noise level"
    )
    ...
    explain_parser.add_argument(
        "--probes", required=True, help="Directory with probe trajectories"
    )
    explain_parser.add_argument(
        "--dataset", choices=["adult", "synthetic", "scenario", "california"], default="adult"
    )
    explain_parser.add_argument(
        "--targets", default="0", help="Target device indices (comma-separated)"
    )
    explain_parser.add_argument(
    ...
    explain_parser.add_argument(
        "--tau", type=float, default=0.5, help="Kernel bandwidth"
    )
    explain_parser.add_argument(
        "--lam", type=float, default=0.01, help="Ridge regularization"
    )
    explain_parser.add_argument(
        "--mode", choices=["p", "s"], default="p", help="Mode: p (proxy) or s (secure)"
    )
    explain_parser.add_argument(
        "--baselines", default="", help="Baselines to run (comma-separated)"
    )
    ...
    synth_parser.add_argument(
        "--p", type=int, default=8, help="Number of features (including intensity slot)"
    )
    synth_parser.add_argument("--N", type=int, default=10, help="Number of devices")
    synth_parser.add_argument(
        "--m", type=int, default=1000, help="Number of probe samples"
    )
    synth_parser.add_argument(
        "--alpha", type=float, default=0.7, help="Treatment strength for target device"
    )
    synth_parser.add_argument(
        "--theta0", default="[0.0,0.2,0.4]", help="Pre-window intensities as list"
    )
    synth_parser.add_argument(
        "--theta", default="[0.6,0.8,1.0,1.2]", help="Post-grid intensities as list"
    )
    synth_parser.add_argument(
        "--K", type=int, default=50, help="Number of anchors for matching"
    )
    synth_parser.add_argument(
        "--tau", type=float, default=0.5, help="Kernel bandwidth for anchors"
    )
    synth_parser.add_argument(
        "--lam", type=float, default=0.01, help="Ridge regularization for matching"
    )
    synth_parser.add_argument(
        "--mode",
        choices=["p", "s"],
        default="p",
        help="DISCO mode: proxy (p) or secure (s)",
    )
    synth_parser.add_argument(
        "--base-jitter",
        type=float,
        default=0.3,
        help="Stddev of device-specific base offsets",
    )
    synth_parser.add_argument(
        "--peer-alpha-std",
        type=float,
        default=0.1,
        help="Stddev of peer treatment strengths",
    )
    synth_parser.add_argument(
        "--noise-std",
        type=float,
        default=0.05,
        help="Observation noise scale in generator",
    )
